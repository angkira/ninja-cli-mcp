...
class RepoTaskQueue:
    ...
    async def execute_with_lock(
        self, repo_root: str | Path, func: Callable[..., T], *args: Any, **kwargs: Any
    ) -> T:
        """
        ...
        async with self._lock:
            ...
            if waiting > 0:
                logger.info(f"[{repo_key}] Task queued, {waiting} task(s) ahead")
    ...
    def get_queue_status(self, repo_root: str | Path) -> dict[str, Any]:
        ...
        return {
            ...
            "locked": self._semaphores[repo_key].locked()
            if repo_key in self._semaphores
            else False,
        }
