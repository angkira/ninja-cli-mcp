...
class FormatFileCommand(HookCommand):
    ...
    def execute(self) -> HookResult:
        ...
        if not path.exists():
            return HookResult(status="error", message=f"File not found: {self.file_path}")
    ...
    def _format_python(self) -> HookResult:
        ...
        exit_code, _stdout, stderr = run_subprocess(cmd, timeout=30.0)
    ...
    def _format_js_ts(self) -> HookResult:
        ...
        exit_code, _stdout, _stderr = run_subprocess(cmd, timeout=30.0)
...
class LintCheckCommand(HookCommand):
    ...
    def execute(self) -> HookResult:
        ...
        exit_code, stdout, _stderr = run_subprocess(cmd, cwd=self.repo_root, timeout=60.0)
        ...
        if exit_code == 0:
            return HookResult(status="pass", data={"issues": [], "total_issues": 0, "fixed": 0})
        else:
            formatted_issues = [
                {
...
class PreCommitCommand(HookCommand):
    ...
    def execute(self) -> HookResult:
        ...
        for f in staged_files:
            if f.endswith(".py"):
                format_cmd = FormatFileCommand(str(Path(self.repo_root) / f), check_only=True)
        ...
        return HookResult(status="pass" if all_passed else "fail", data={"checks": checks})


def main() -> int:
    """Main entry point for hooks CLI."""
    parser = argparse.ArgumentParser(
        description="Ninja Coder Hooks CLI", formatter_class=argparse.RawDescriptionHelpFormatter
    )
