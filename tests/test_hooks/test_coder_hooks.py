...
class TestLintCheckCommand:
    ...
    @patch("ninja_coder.hooks_cli.run_subprocess")
    @patch("ninja_coder.hooks_cli.get_staged_files")
    def test_lint_check_json_decode_error(self, mock_get_staged_files, mock_run_subprocess, tmp_path):
...
class TestPreCommitCommand:
    ...
    def test_precommit_all_pass(self, mock_format_execute, mock_lint_execute, mock_get_staged_files, tmp_path):
        """Test pre-commit when all checks pass."""
        # Mock dependencies
        mock_get_staged_files.return_value = ["src/test.py"]
        mock_lint_execute.return_value = type('obj', (object,), {'status': 'pass', 'success': True, 'data': {'total_issues': 0}})
        mock_format_execute.return_value = type('obj', (object,), {'status': 'pass', 'success': True})
    ...
    @patch("ninja_coder.hooks_cli.LintCheckCommand.execute")
    @patch("ninja_coder.hooks_cli.FormatFileCommand.execute")
    def test_precommit_lint_fail(self, mock_format_execute, mock_lint_execute, mock_get_staged_files, tmp_path):
        """Test pre-commit when lint check fails."""
        # Mock dependencies
        mock_get_staged_files.return_value = ["src/test.py"]
        mock_lint_execute.return_value = type('obj', (object,), {'status': 'fail', 'success': False, 'data': {'total_issues': 2}})
        mock_format_execute.return_value = type('obj', (object,), {'status': 'pass', 'success': True})
    ...
    @patch("ninja_coder.hooks_cli.LintCheckCommand.execute")
    @patch("ninja_coder.hooks_cli.FormatFileCommand.execute")
    def test_precommit_format_fail(self, mock_format_execute, mock_lint_execute, mock_get_staged_files, tmp_path):
        """Test pre-commit when format check fails."""
        # Mock dependencies
        mock_get_staged_files.return_value = ["src/test.py"]
        mock_lint_execute.return_value = type('obj', (object,), {'status': 'pass', 'success': True, 'data': {'total_issues': 0}})
        mock_format_execute.return_value = type('obj', (object,), {'status': 'fail', 'success': False})
...
class TestCLIIntegration:
    ...
    def test_main_lint_check_staged_fix(self, mock_lint_command):
        ...
        # Simulate command line arguments
        with patch("sys.argv", ["hooks_cli.py", "lint-check", "--staged", "--fix", "--repo-root", "/test"]):
            result = main()
    ...
    def test_main_no_command(self):
        ...
        with patch("sys.argv", ["hooks_cli.py"]), \
             patch("sys.stderr"), \
