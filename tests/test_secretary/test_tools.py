...
class TestAnalyseFile:
    ...
    @pytest.mark.asyncio
    async def test_analyse_file_success(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
...
"""
    ...
    @pytest.mark.asyncio
    async def test_analyse_file_with_search_pattern(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
...
"""
        ...
        request = AnalyseFileRequest(file_path=str(test_file), search_pattern=r"calculate_\w+")
        result = await executor.analyse_file(request, client_id="test")
    ...
    @pytest.mark.asyncio
    async def test_analyse_file_structure_extraction(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
...
"""
    ...
    @pytest.mark.asyncio
    async def test_analyse_file_language_detection(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
...
class TestFileSearch:
    ...
    async def test_file_search_glob_pattern(
        ...
        request = FileSearchRequest(pattern="*.py", repo_root=str(temp_dir))
    ...
    async def test_file_search_max_results(
        ...
        request = FileSearchRequest(pattern="*.py", repo_root=str(temp_dir), max_results=5)
        result = await executor.file_search(request, client_id="test")
    ...
    @pytest.mark.asyncio
    async def test_file_search_nested_pattern(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
        ...
        file_names = [Path(match.path).name for match in result.matches]
    ...
    async def test_file_search_no_matches(
        ...
        request = FileSearchRequest(pattern="*.java", repo_root=str(temp_dir))
        result = await executor.file_search(request, client_id="test")
...
class TestCodebaseReport:
    ...
    @pytest.mark.asyncio
    async def test_codebase_report_metrics(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_codebase_report_structure(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_codebase_report_markdown_format(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
...
class TestDocumentSummary:
    ...
    @pytest.mark.asyncio
    async def test_document_summary_finds_readmes(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_document_summary_multiple_docs(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_document_summary_custom_patterns(
        self, temp_dir: Path, executor: SecretaryToolExecutor
    ) -> None:
        ...
        request = DocumentSummaryRequest(repo_root=str(temp_dir), doc_patterns=["**/*.txt"])
        result = await executor.document_summary(request, client_id="test")
